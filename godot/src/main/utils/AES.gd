...
static func encrypt(plaintext, key, iv = null, pad_type = PadType.PKCS7):
...
static func decrypt(ciphertext, key, iv = null, pad_type = PadType.PKCS7):
...
static func _encrypt_block(block, keys):
	block = [
...
static func _decrypt_block(block, keys):
	block = [
		PoolByteArray([block[0x00], block[0x01], block[0x02], block[0x03]]),
		PoolByteArray([block[0x04], block[0x05], block[0x06], block[0x07]]),
		PoolByteArray([block[0x08], block[0x09], block[0x0a], block[0x0b]]),
...
static func _sub(byte, s_box):
	return s_box[byte / 16][byte % 16]

static func _mix_columns(block, mix_matrix):
	for i in range(4):
...
static func _to_raw_array(object):
	match typeof(object):
		TYPE_RAW_ARRAY:
			return object
		TYPE_ARRAY:
